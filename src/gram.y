...
%token      AFISEAZA
			...
			/* token-uri sintaxa */
%token<num> PARANTEZA_S PARANTEZA_D	DOUA_PCT
%token<num> EOL
...
structura_for: FOR PARANTEZA_S astElem NEXT PARANTEZA_D DOUA_PCT astElem 	
																			{ 
																			makeWhile($3, $7);
																			}
			| FOR PARANTEZA_S expr NEXT PARANTEZA_D DOUA_PCT linie_execs_compus 			
			;	
...
structura_if: IF comparare linie_exec ELSE linie_exec 				{ if ((int)$2) printf("if"); else printf("%d\n", (int)$2); }
			| IF expr linie_execs_compus ELSE linie_execs_compus 
			;
...
afisare_ecran:	AFISEAZA VARIABILA { 
		...
		| AFISEAZA NUMAR	 	{ printf ("%f\n", (float)$2); }
		| AFISEAZA comparare	{ if ((int)$2 == 1) 
									printf("Adevarat\n");
								  else
								  	printf("Fals\n");	
								}
...
atribuire: 
		...
		;
atribuire_str:
		VARIABILA_STR EGAL CUVANT					{ $$ = set_variabila_str($1, $3); }
%%
